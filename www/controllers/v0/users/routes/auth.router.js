...
router.post('/login', (req, res) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (!user) {
        return res.status(401).send({ auth: false, message: 'Unauthorized, no user' });
    }
    // check that the password matches
    const authValid = yield comparePasswords(password, user.password_hash);
    if (!authValid) {
        return res.status(401).send({ auth: false, message: 'Unauthorized, bad password' });
    }
    // Generate JWT
